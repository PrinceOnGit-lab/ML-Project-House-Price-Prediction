...
# User input fields
location = st.selectbox('Location', ['Select'] + list(locations))
...
with col2:
    bhk = st.number_input('Number of Bedrooms (BHK)', min_value=0, max_value=10, value=2)

# Prediction logic with input validation
if st.button('Predict'):
    if location == 'Select' or total_sqft <= 0 or bath <= 0 or bhk <= 0:
        st.warning("⚠️ Please fill in all the fields with valid values before predicting.")
    else:
        try:
